...
const trendingReposList = document.getElementById('trendingReposList');
...
fetchTrendingBtn.addEventListener('click', fetchTrendingRepos);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const section = urlParams.get('section') || 'search';
    ...
    // Load some trending repos by default
    if (section === 'trending') {
        fetchTrendingRepos();
    }
});
...
function animateHeroElements() {
    const heroElements = document.querySelectorAll('.hero-title, .hero-subtitle, .search-container, .filter-container');
...
async function searchUser() {
    ...
    try {
        ...
        // Update URL
        const newUrl = `${window.location.pathname}?section=search&user=${username}`;
        window.history.pushState({ path: newUrl }, '', newUrl);
...
async function fetchTrendingRepos() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/search/repositories?q=${encodeURIComponent(query)}&sort=stars&order=desc&per_page=12`);
        ...
        displayTrendingRepos(data.items || []);
        
    } catch (error) {
        showTrendingError(error.message);
    } finally {
        hideTrendingLoader();
    }
}
...
function displayTrendingRepos(repos) {
    if (repos.length === 0) {
        trendingReposList.innerHTML = '<p class="no-repos">No trending repositories found for the selected criteria</p>';
        return;
    }
    
    trendingReposList.innerHTML = repos.map((repo, index) => `
        <div class="repo-card" style="animation-delay: ${index * 0.05}s">
            ...
            <div class="repo-stats">
                ...
                ` : ''}
                <span class="repo-stat" title="Owner">
                    <svg class="repo-stat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
function shareProfile() {
    const username = searchInput.value.trim();
    const url = `${window.location.origin}${window.location.pathname}?section=search&user=${username}`;
...
function hideTrendingError() {
    const errorElements = trendingReposList.querySelectorAll('.error-message');
